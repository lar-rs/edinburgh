...
pub fn average(path:PathBuf, address:String) -> io::Result<()> {

    let c = can::io::connection(address.as_str())?;
    ...
    let mut signal = LineWriter::new(fs::File::create(path.join("signal"))?);
    ...
    for _c in 0..counter {
        ...
        for v in values {
            ...
            if let Some(fsr) = extract_fsr(v) {
                ...
                signal.write_all(format!("{},{}",tm.as_millis(),fsr).as_bytes())?;
                write!(signal,"{},{}\n",tm.as_millis(),fsr)?;
                fs::write(path.join("value"),format!("{}",fsr).as_bytes())?;
                println!("{},{}",tm.as_millis(),fsr);
...
impl Average {
    pub fn run(&self,dir:&Path,address:&str) -> io::Result<()> {
        if let Err(e) = average(dir.to_path_buf(),address.to_owned()) {
            eprintln!("{}", e)
        }
        Ok(())
    }
